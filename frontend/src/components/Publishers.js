...
const Publishers = () => {
  ...
  const fetchPublishers = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8000/api/publishers/?page=${page}&limit=${limit}&sort_by=${sortBy}`);
      ...
      <div className="container">
        ...
        <div className="controls">
          ...
          <div className="page-info">
            Page {page} of {totalPages} ({totalPublishers} total publishers)
          </div>
        </div>
        ...
        <div className="grid">
          {publishers.map(publisher => (
            <div key={publisher.id} className="card">
              ...
              <button className="btn-pill-link" onClick={() => navigate(`/publishers/${publisher.id}`)}>View Details</button>
            </div>
          ))}
        </div>
